...
class User:
    ...
    def print_info(self):
        ...
        _posts = [post for post in posts if post.author == self.login]
...
class Registration:
    ...
    def register(self, login, password, password_verification, is_admin=False):
        if self._login_validation(login) and self._password_validation(password, password_verification):
            users.append(User(login, password, is_admin))
...
registration_object.register('1', '1q', '1q', True)
        ...
        if login_object.logged:
            while True:
                ...
                if code == 0:
                    ...
                    title = input("Title: ")
                    data = input("Post: ")
                    posts.append(Post(login_object.logged.login, title, data))
                elif code == 2:
                    _posts = [post for post in posts if post.author == login_object.logged.login]
                    for post in _posts:
                        post.print_info()
